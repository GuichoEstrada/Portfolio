...
const CustomLink = ({href,title,className=""}) => {
    ...
    return (
        <Link href={href} className={`${className} relative group`}>
            {title}
            <span className={`h-[1px] inline-block w-0 bg-dark absolute 
...
const CustomMobileLink = ({href,title,className="", toggle}) => {
    ...
    return (
        <button href={href} className={`${className} relative group text-light dark:text-dark my-2`} onClick={handleClick}>
            {title}
            <span className={`h-[1px] inline-block w-0 bg-light dark:bg-dark absolute 
            left-0 -bottom-0.5 group-hover:w-full transition-[width] 
            ease duration-300
...
const NavBar = () => {
  ...
  return (
    ...
    >
        <button className='flex-col justify-center items-center hidden lg:flex' onClick={toggleHamburgerMenu}>
        ...
        <div className='w-full flex justify-between items-center lg:hidden'>
            ...
            <nav className='flex items-center justify-center flex-wrap'>
                <motion.a href="https://github.com/GuichoEstrada" target={"_blank"} 
                className='w-6 mr-3'
                whileTap={{scale:0.9}}
                whileHover={{y:-2}}
                >
                ...
                <motion.a href="https://www.linkedin.com/in/lcestrada/" target={"_blank"} 
                className='w-6 mx-3'
                whileTap={{scale:0.9}}
                whileHover={{y:-2}}
                >
                ...
                <motion.a href="https://twitter.com/wissestrada" target={"_blank"} 
                className='w-6 mx-3'
                whileTap={{scale:0.9}}
                whileHover={{y:-2}}
                >
            ...
            <motion.div 
                ...
                >
                    <nav className='flex items-center flex-col justify-center'>
                        <CustomMobileLink href="/" title="Home" className='mx-4' toggle={toggleHamburgerMenu}/>
                        <CustomMobileLink href="/about" title="About" className='mx-4' toggle={toggleHamburgerMenu}/>
                        <CustomMobileLink href="/projects" title="Projects" className='mx-4' toggle={toggleHamburgerMenu}/>
                    </nav>
                    ...
                    <nav className='flex items-center justify-center flex-wrap mt-8'>
                        ...
                        <motion.a href="https://www.linkedin.com/in/lcestrada/" target={"_blank"} 
                        className='w-6 mx-3 dark:bg-dark sm:mx-1'
                        whileTap={{scale:0.9}}
                        whileHover={{y:-2}}
                        >
                            <LinkedInIcon />
                    ...
                    <button 
                        onClick={() => setMode(mode === "light" ? "dark" : "light")}
                        className={`mt-4 flex items-center justify-center rounded-full p-1 ${mode==="light" ? "bg-dark text-light" : "bg-light text-dark sm:mx-1"}`}
                    >
                        {
                            mode === "dark" ?
                            <SunIcon className={"fill-dark"}/>
